...
# Attack config
parser.add_argument(
    '--attack_dir', type=str, default='pcba_adaptive', help='attack folder')
parser.add_argument(
...
parser.add_argument('--adaptive_weight', type=float, default=0.)
...
opt.attack_dir = os.path.join(opt.attack_dir, str(opt.adaptive_weight),opt.dataset)
...
bd_corrupt_class = corrupt(seed=opt.manualSeed, origin_num=1025)
corruption=["background", "cutout", "density", "density_inc", "distortion", \
            "distortion_rbf", "distortion_rbf_inv", "gaussian", "impulse", \
            "rotation", "scale", "shear", "uniform", "upsampling", "ufsampling"]
...
for t in range(opt.n_init):
    ...
    for iter in tqdm(range(opt.NSTEP)):
        ...
        ori_points, labels = points.to(opt.device), labels.to(opt.device)
        centers_copies = centers_trial.repeat(len(points), 1, 1)
        points = torch.cat([ori_points, centers_copies], dim=1)
        points = points.transpose(2, 1)
        pred = classifier(points)
        # Check if stopping criteria is satisfied
        posterior = torch.exp(pred).detach().cpu()
        ...
        #! ======================================
        ori_pred = classifier(ori_points.transpose(2,1))
        ori_outputs = []
        bd_outputs = []
        for c in corruption:
            corrupt = corrupt_class(c)
            bd_corrupt = bd_corrupt_class(c)
            flag=0

            for s in range(1,6):
                corrupt_ori_pc = corrupt(ori_points.clone().detach().squeeze(0).cpu().numpy(), s)
                corrupt_ori_pc = torch.tensor(corrupt_ori_pc).cuda().unsqueeze(0).transpose(2, 1)
                ori_output = classifier(corrupt_ori_pc)

                if ori_output.argmax() != ori_pred.argmax():
                    ori_outputs.append(s+1)
                    flag = 1
                    break
            if flag == 0:
                ori_outputs.append(6)

            flag=0
            for s in range(1,6):
                corrupt_bd_pc = bd_corrupt(points.clone().detach().transpose(2, 1).squeeze(0).cpu().numpy(), s)
                corrupt_bd_pc = torch.tensor(corrupt_bd_pc).cuda().unsqueeze(0).transpose(2, 1)
                bd_output = classifier(corrupt_bd_pc)

                if ori_output.argmax() != pred.argmax():
                    bd_outputs.append(s+1)
                    flag = 1
                    break
            if flag == 0:
                bd_outputs.append(6)
        ...
        def KL_Divergence(a, b):
            assert len(a) == len(b), print(len(a), len(b))
            result = 0
            a /= np.linalg.norm(a)
            b /= np.linalg.norm(b)
            ...
            for i in range(len(a)):
                if a[i] == 0 or b[i] == 0:
                    ...
                    result += a[i] * np.log2(abs(a[i] / b[i]))
            return result

        adaptive_loss = KL_Divergence(ori_outputs, bd_outputs)
        loss = F.nll_loss(pred, labels.view(-1).long()) + opt.adaptive_weight * adaptive_loss
        #! ======================================
